<template>
  ...
  <div>
    <div style="margin-bottom: 2rem">
      ...
      <button style="margin-left: 2rem" v-on:click="fireWorker" class="rm">FIRE</button>
      <button style="margin-left: 2rem" v-on:click="hireWorker" class="rm">HIRE</button>
...
export default {
  ...
  methods: {
    ...
    fireWorker(){
      ...
      this.$emit('fire-workers',this.id)
    },

    hireWorker(){
      let paramsURI = 'hire/';
      paramsURI += this.updWorker.id + '/' + this.organization + '/' + this.position + '/' + this.startDate
      console.log(paramsURI)
      this.$emit('hire-workers', paramsURI)
    },
    ...
    addWorker(){
      ...
      if (this.xCoord !== '') this.addParams.push('xCoord='.concat(this.xCoord)) // </>/=value
      if (this.yCoord !== '') this.addParams.push('yCoord='.concat(this.yCoord)) // </>/=value
