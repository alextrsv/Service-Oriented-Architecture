...
export default {
  ...
  data() {
    return{
      ...
      page: 0,
      size: 2,
      url : 'https://localhost:8762/workerapi/workers',
      finalUrl: '',
      params : [''],
      filterParams : [''],
      sortParams : '',
  ...
  methods: {
    removeWorker() {
      ...
      this.finalUrl = this.url + '?id=' + this.currentWorker.id
      fetch(this.finalUrl, {
        method: 'DELETE'})
          .then(res => {
            if (!res.ok){
    ...
    getMaxPos(){
      fetch("https://localhost:8762/workerapi/workers/filter/maxPos")
          .then(res => {
            if (!res.ok){
              res.json().then(json => this.message = json.detailMessage)
            }
    ...
    getAmountByDate(filterParams){
      this.filterParams = []
      this.filterParams = Object.assign(filterParams);
      let dateAmountUrl =  'https://localhost:8762/workerapi/workers/date/amount?' + filterParams[0]
    ...
    getAmountByStatus(filterParams){
      this.filterParams = []
      this.filterParams = Object.assign(filterParams);
      let dateAmountUrl =  'https://localhost:8762/workerapi/workers/status/amount?' + filterParams[0]
