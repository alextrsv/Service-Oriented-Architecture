...
export default {
  ...
  methods: {
    ...
    getMaxPos(){
      fetch("http://localhost:8080/workers/filter/maxPos")
          .then(res => {
            if (!res.ok){
              res.json().then(json => this.message = json.detailMessage)
            }
    ...
    getAmountByDate(filterParams){
      this.filterParams = []
      this.filterParams = Object.assign(filterParams);
      let dateAmountUrl =  'http://localhost:8080/workers/date/amount?' + filterParams[0]
    ...
    getAmountByStatus(filterParams){
      this.filterParams = []
      this.filterParams = Object.assign(filterParams);
      let dateAmountUrl =  'http://localhost:8080/workers/status/amount?' + filterParams[0]
